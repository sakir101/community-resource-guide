...
import { ArrowLeft, Search, Heart, MapPin, Calendar, MessageSquare, Filter } from "lucide-react"
...
export default function HamaspikProgramsPage() {
  ...
  const [selectedProgramForFeedback, setSelectedProgramForFeedback] = useState<HamaspikProgram | null>(null)
  ...
  const [selectedFunctioningLevel, setSelectedFunctioningLevel] = useState("all")
  ...
  useEffect(() => {
    const fetchHamaspikPrograms = async () => {
      try {
        const response = await fetch("/api/resources?category=hamaspik-programs")
        const result = await response.json()
        if (result.success) {
          setHamaspikData(result.data)
        }
      } catch (error) {
        console.error("Failed to fetch hamaspik programs:", error)
      } finally {
        setLoading(false)
      }
    }
  ...
  const getAvailableAges = () => {
    ...
    hamaspikData.forEach((program) => {
      if (program.gender) {
        ...
        if (genderLower.includes("3") || genderLower.includes("4") || genderLower.includes("5")) ages.add("3-9")
        if (genderLower.includes("9") || genderLower.includes("10") || genderLower.includes("11")) ages.add("9-15")
        if (genderLower.includes("15") || genderLower.includes("16") || genderLower.includes("17")) ages.add("15-21")
        if (genderLower.includes("21") || genderLower.includes("adult")) ages.add("21+")
      }
    })
    return [
  ...
  const getAvailableGenders = () => {
    ...
    hamaspikData.forEach((program) => {
      if (program.gender) {
        const genderLower = program.gender.toLowerCase()
        if (genderLower.includes("male") || genderLower.includes("boys")) genders.add("male")
        if (genderLower.includes("female") || genderLower.includes("girls")) genders.add("female")
    ...
    return [
      ...
      ...Array.from(genders).map((gender) => ({
        value: gender,
        label: gender === "male" ? "Male" : gender === "female" ? "Female" : "Both",
      })),
    ]
  }
  ...
  const getAvailableLocations = () => {
    ...
    hamaspikData.forEach((program) => {
      if (program.location) {
        ...
        if (locationLower.includes("bp") || locationLower.includes("boro park")) locations.add("bp")
        if (locationLower.includes("williamsburg")) locations.add("williamsburg")
        if (locationLower.includes("flatbush")) locations.add("flatbush")
        if (locationLower.includes("crown heights")) locations.add("crown-heights")
        if (locationLower.includes("far rockaway")) locations.add("far-rockaway")
        if (locationLower.includes("staten island")) locations.add("staten-island")
      }
    })
    return [
  ...
  const getAvailableFunctioningLevels = () => {
    ...
    hamaspikData.forEach((program) => {
      if (program.functioningLevel) {
        ...
        if (levelLower.includes("various") || levelLower.includes("all")) levels.add("various")
      }
    })
    return [
  ...
  const getAvailableDays = () => {
    ...
    hamaspikData.forEach((program) => {
      if (program.daysOpen) {
        ...
        if (daysLower.includes("all") || daysLower.includes("24/7")) days.add("all-week")
        if (daysLower.includes("flexible") || daysLower.includes("as needed")) days.add("flexible")
      }
    })
    return [
  ...
  const matchesLocationFilter = (location: string, selectedLocation: string): boolean => {
    ...
    switch (selectedLocation) {
      case "bp":
        return locationLower.includes("bp") || locationLower.includes("boro park")
      case "williamsburg":
  ...
  const matchesFunctioningLevelFilter = (functioningLevel: string, selectedLevel: string): boolean => {
  ...
  const matchesDaysFilter = (daysOpen: string, selectedDays: string): boolean => {
    ...
    switch (selectedDays) {
      ...
      case "flexible":
        return daysLower.includes("flexible") || daysLower.includes("as needed")
      default:
        return true
    }
  }
  ...
  const filteredPrograms = hamaspikData.filter((program) => {
    ...
    const matchesAge =
      selectedAge === "all" ||
      (program.gender && program.gender.toLowerCase().includes(selectedAge.toLowerCase().split("-")[0])) ||
    ...
    const matchesLocation = matchesLocationFilter(program.location, selectedLocation)
    const matchesFunctioningLevel = matchesFunctioningLevelFilter(program.functioningLevel, selectedFunctioningLevel)
    const matchesDays = matchesDaysFilter(program.daysOpen, selectedDays)

    return matchesSearch && matchesAge && matchesGender && matchesLocation && matchesFunctioningLevel && matchesDays
  })
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Hamaspik Programs</h1>
          <p className="text-gray-600">Hamaspik organization programs and services</p>
        </div>
        ...
        <div className="mb-6 space-y-4">
          ...
          <div className="bg-white p-4 rounded-lg border shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              ...
              <div>
                <label className="text-sm font-medium text-gray-700 mb-1 block">Functioning Level</label>
                <Select value={selectedFunctioningLevel} onValueChange={setSelectedFunctioningLevel}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div>
                <label className="text-sm font-medium text-gray-700 mb-1 block">Days Open</label>
        ...
        <Dialog open={feedbackDialog} onOpenChange={setFeedbackDialog}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>Suggest Update for {selectedProgramForFeedback?.program}</DialogTitle>
              <p className="text-sm text-gray-600">
        ...
        <div className="mt-8 bg-blue-50 border border-blue-200 rounded-lg p-6">
          ...
          <ul className="text-blue-800 space-y-2 text-sm">
            <li>
              • Most programs have transportation from all areas including Boro Park, Flatbush, Crown Heights, Staten
              Island, and Far Rockaway
            </li>
            <li>• Programs offer full day services on legal holidays, Chol HaMoed, and Isru Chag</li>
            <li>• Shabbatons are offered 2-3 times per year</li>
            <li>• Fit with Friends program has Shabbos programs in Flatbush and Far Rockaway</li>
          </ul>
        </div>
      </div>
    </div>
